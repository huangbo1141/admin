...
public class AppApiHelper extends BaseHelperImpl{
	...
	public Object login(RestRequest apiRequest){
		...
		if(whereFields.containsKey("serial")&&whereFields.containsKey("password")){
			...
			if(list.size()>0){
				...
				String sql = "select * from tbl_announce "
						+" order by time desc";
	...
	public Object get_workorder(RestRequest apiRequest){
		...
		if(wf.containsKey("dan")&&wf.containsKey("current_date")){
			...
			String sql = "select O.* from tbl_order as O"
					...
					+" WHERE U.dan = "+dan
					+" and substring(O.create_datetime,1,10) = '"+temp_date+"'";
	...
	public Object workorder_pos(RestRequest apiRequest){
		...
		if(wf.containsKey("station_id")){
			...
			HashMap<String,Object> ret = new HashMap<String,Object>();
			ret.put("message", "station_id not come");
			return ret;
		}
	}
	...
	public HashMap<String,Object> getOrderInfo(Order order,int mode){
		...
		if(mode == 1){
			...
			String sql = "select " + str_select
					+" from "+this.getTableName(OrderRelation.class.getSimpleName())
...
//						return rhs.getId() > lhs.getId() ? -1 : (rhs.getId() < lhs.getId()) ? 1 : 0;
					}
	...
	public Object create_workorder(RestRequest apiRequest){
		...
		if(wf.containsKey("receiver_id")){
			...
			if(model_order!=null){
				...
				Object model_or = this.modelNew(OrderRelation.class, this.orderService, this.getHashMapOfObject(or, OrderRelation.class,null), 1);
				if(model_or!=null){
					HashMap<String,Object> ret = new HashMap<String,Object>();
	...
	public Object order_update(RestRequest apiRequest){
		...
		if(wf.size()>0){
			...
			if(!object.equals("")){
				if(reason_id.equals("3")){
					// this is p
					String sql = "select * from tbl_order_relation "
	...
	public String getNearestTime(){
		...
		String T = null;
		if(temp.size()>0){
			String report_id = temp.get(0).toString();
			T = temp.get(1).toString();
		}
		session.close();
		return T;
	}
	public List getReportForPeriod(String T1,String T2,Integer userid){
		String sql = "select R.* from tbl_report as R"
				+" where substring(R.create_datetime,1,10) >= '"+T1+"'"
				+" and substring(R.create_datetime,1,10) <= '"+T2+"'";
		if(userid!=null){
			sql = sql + " and R.user_id ="+userid;
		}
		List list= this.sqlQuery(Report.class, this.reportService, sql, 1);
		return list;
	}
	...
	public Object get_line_report(RestRequest apiRequest){
		...
		if(wf.containsKey("mode")){
			...
			if (mode.equals("1")){
				...
				HashMap<Integer,Object> map_data = this.processUserForReport(list_report, T);
				//get tt value
				if(T!=null){
					sql = "select T.* from tbl_tt as T"
							+" where time_value = '"+ T.substring(0,10)+ "'"
				...
				if(wf.containsKey("T1")&&wf.containsKey("T2")&&wf.containsKey("time_type")&&wf.containsKey("time_value"))
				{
					...
					// this is report table
					HashMap map_data = this.processUserForReport(list_report, T);
	...
	public HashMap<Integer,Object> processUserForReport(List list,String T){
		// ASSUME LIST REPORT AS SAME DAYS
		HashMap<Integer,Object> map_data = new HashMap<Integer,Object>();
		for(Object obj:list){
			Report report = (Report)obj;
			String sql = "select sum(TIMESTAMPDIFF(MINUTE, O.create_datetime, O.complete)) as sum1"
					+" from tbl_order as O"
...
//					order_time = sum1.intValue();
			...
			if(map_data.containsKey(line.getId())){
				List<TaOee> ilist = (List<TaOee>)map_data.get(line.getId());
				...
				taoee.setOee(oee);
				ilist.add(taoee);
				map_data.put(line.getId(), ilist);
			}
		}

		//
		
		HashMap<Integer,Object> map_ret = new HashMap<Integer,Object>();
		...
		while (it.hasNext()) {
			...
			hash.put("line", this.getHashMapOfObject(line, Line.class,null));
			hash.put("cal_data", taoee);
			
			//list_data.add(hash);
			map_ret.put(key, hash);
		}
		return map_ret;
	}
	...
	public Object search(RestRequest apiRequest){
		...
		if(wf.containsKey("key")){
			...
			String term = "%"+key+"%";
			String sql = "select U.* from tbl_user as U "
	...
	public Object assign_workorder(RestRequest apiRequest){
		HashMap<String, Object> wf = (HashMap<String, Object>)apiRequest.model;
		if(wf.containsKey("sender_id")&&wf.containsKey("receiver_id")
				&&wf.containsKey("order_id")&&wf.containsKey("s_time")){
			// 	&&wf.containsKey("r_time")
			Object model = this.modelNew(OrderRelation.class, this.orderRelationService, apiRequest.model,1);
			if(model!=null){
				HashMap<String,Object> ret = new HashMap<String,Object>();
				ret.put("model", model);
				ret.put("response", 200);
			...
			HashMap<String,Object> ret = new HashMap<String,Object>();
			ret.put("message", "sender_id receiver_id order_id s_time not come");
			return ret;
		}
		return null;
	}
	...
	public Object get_workorder_create_info(RestRequest apiRequest){
		...
		if(wf.containsKey("id")){
			...
			String sql = "SELECT * FROM `tbl_user`"
					+" where dan="+dan.getId()+" and type = " + ur_main.getId();
			...
			sql = "SELECT * FROM `tbl_user`"
					+" where dan="+dan.getId()+" and type = " + ur_quality.getId();
			List list_quality = this.sqlQuery(User.class, this.userService, sql,	1);
						
			sql = "SELECT * FROM `tbl_user`"
					+" where dan="+dan.getId()+" and type = " + ur_specialist.getId();
	...
	public Object workorder_list(RestRequest apiRequest){
		HashMap<String, Object> wf = (HashMap<String, Object>)apiRequest.model;
		if(wf.containsKey("start_date")&&wf.containsKey("end_date")){
			...
			// get workorder for dan
			String sql = "select O.* from tbl_order as O"
					+" where substring(O.create_datetime,1,10) >= '"+start_date+"'"
					+" and substring(O.create_datetime,1,10) <= '"+end_date+"'"
					;
			List list = this.sqlQuery(Order.class, this.orderService, sql, 1);
			HashMap<String,Object> ret = new HashMap<String,Object>();
			List<Object> list_data = new ArrayList<Object>();
	...
	public Object get_order_for_worker(RestRequest apiRequest){
		...
		if(wf.containsKey("id")){
			...
			// get workorder for dan
			String sql = "select O.* from tbl_order as O"
					+" join tbl_order_relation as R on O.id = R.order_id"
					+" where R.receiver_id= '"+id+"'"
					;
			List list = this.sqlQuery(Order.class, this.orderService, sql, 1);
			
			HashMap<Integer,Object> map_order = new HashMap<Integer,Object>();
			map_order.put(0, new ArrayList<Object>());
			map_order.put(1, new ArrayList<Object>());
			map_order.put(2, new ArrayList<Object>());
			for(Object obj:list){
				Order order = (Order)obj;
				Integer type = order.getStatus();
				if(map_order.containsKey(type)){
					//
					List<Object> ilist = (List<Object>)map_order.get(type);
					ilist.add(this.getOrderInfo(order, 0));
					
				}else{
					//
					List<Object> ilist = new ArrayList<Object>();
					ilist.add(this.getOrderInfo(order, 0));
					map_order.put(type, ilist);
				}
			}
			HashMap<String,Object> ret = new HashMap<String,Object>();
			ret.put("list0", map_order.get(0));
			ret.put("list1", map_order.get(1));
			ret.put("list2", map_order.get(2));
			ret.put("response", 200);
			return ret;
		}else{
			HashMap<String,Object> ret = new HashMap<String,Object>();
	...
	public Object get_error_type(RestRequest apiRequest){
		...
		if(wf.containsKey("reason_id")){
			...
			// get workorder for dan
			String sql = "select * from tbl_error_type "
	...
	public Object get_all_user(RestRequest apiRequest){
		...
		for(Line line:list_line){
			String sql = "select U.* from tbl_user as U "
					...
					+ " JOIN tbl_line as L ON L.id = D.line"
					+ " where L.id ="+line.getId();
	...
	public HashMap<String,Object> processMultipartRequest(HttpServletRequest request,
			String term,String v,List<String> file_names) {
		HashMap<String,Object> ret = new HashMap<String,Object>();
		ret.put("response", 400);
		...
		if(term.equals("set_head")){
			...
			non_safe.put("reason_id", reason_id);
